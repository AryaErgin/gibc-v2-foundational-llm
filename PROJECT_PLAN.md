...
EXP-011 completed the authorized fresh seed-42 long-horizon calibration: 45,777 updates / 1,500,020,736 prediction tokens, with the cosine horizon fixed from step zero. The stream retained exact EXP-004 and EXP-006 raw-byte prefixes and the 2:1 globally deduplicated, contamination-screened Data Recipe v1 mixture. Details: [results/EXP-011-summary.md](results/EXP-011-summary.md).
...
Stop for research review. No official benchmark, additional training, LR/architecture/data change, or EXP-012 is authorized by this document. A future official evaluation may run only on the checkpoint selected by the frozen validation-based promotion rule.
