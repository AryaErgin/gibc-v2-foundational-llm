...
def full_run_milestones(config: ExperimentConfig) -> tuple[int, int, int, int]:
    ...
    if config.experiment_id not in {"EXP-003", "EXP-004", *EXP005_IDS, *EXP007_IDS, *EXP008_IDS, *EXP009_IDS}:
...
def assert_physical_batch_control(config: ExperimentConfig, microbatch_sequences: int, accumulation_steps: int) -> None:
    ...
    if config.experiment_id in {"EXP-003", "EXP-004", *EXP005_IDS, *EXP007_IDS, *EXP008_IDS, *EXP009_IDS, EXP006_ID} and (
...
def load_full_run_artifact(artifact_dir: Path, config: ExperimentConfig) -> FullRunArtifact:
    ...
    expected_manifest_id = "EXP-004" if config.experiment_id in {*EXP005_IDS, *EXP007_IDS, *EXP008_IDS, *EXP009_IDS} else config.experiment_id
    ...
    dual_validation_experiment = config.experiment_id in {"EXP-003", "EXP-004", *EXP005_IDS, *EXP007_IDS, *EXP008_IDS, *EXP009_IDS, EXP006_ID}
    ...
    if config.experiment_id in {*EXP005_IDS, *EXP007_IDS, *EXP008_IDS, *EXP009_IDS} and packed.get("train_stream_sha256") != EXP004_FROZEN_STREAM_SHA256:
        raise RuntimeError("EXP-005/EXP-007/EXP-008/EXP-009 requires the exact frozen EXP-004 stream SHA-256; rematerialized streams are forbidden.")
    ...
    if dual_validation_experiment:
        ...
        expected_edu_hashes = (
            ...
            if config.experiment_id in {"EXP-004", *EXP005_IDS, *EXP007_IDS, *EXP008_IDS, *EXP009_IDS, EXP006_ID}
