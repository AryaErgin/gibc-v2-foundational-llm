...
def assert_physical_batch_control(config: ExperimentConfig, microbatch_sequences: int, accumulation_steps: int) -> None:
    ...
    if config.experiment_id in {"EXP-003", "EXP-004", *EXP005_IDS, EXP006_ID} and (
...
def load_full_run_artifact(artifact_dir: Path, config: ExperimentConfig) -> FullRunArtifact:
    ...
    dual_validation_experiment = config.experiment_id in {"EXP-003", "EXP-004", *EXP005_IDS, EXP006_ID}
    ...
    if dual_validation_experiment:
        ...
        expected_edu_hashes = (
            ...
            if config.experiment_id in {"EXP-004", *EXP005_IDS, EXP006_ID}
