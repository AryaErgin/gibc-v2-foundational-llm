...
## EXP-020 data-preparation attempt 1 — systems-performance abort (2026-09-04)

Status: **INVALID DATA-PREPARATION SYSTEMS ABORT; no model training and no scientific result.** The first deterministic 7.2B rebuild (builder PID 469) was terminated by explicit authorization after live forensics. It had materialized only a defensible lower-bound frontier of 530,579,456 of 7,199,981,569 stored IDs (about 7.36918%) after about 167,484 seconds; continuation at the observed average rate projected about 584.8 further hours. The preallocated final-sized memmap was never evidence of completion, no valid manifest.json existed, and the queued verifier observed exit status 143 rather than authorizing the partial artifact.

Forensics identified source-faithful implementation bottlenecks: per-document normalized 13-gram SQLite screening through the 1.02-GB benchmark index on DrvFS, plus Python-level scalar stream serialization. The partial artifact is explicitly marked INVALID/ABORTED and preserved only for diagnosis; it must never be consumed for training or provenance.

The recovery implementation does not alter datasets, revisions, source order, mixture, global deduplication, stable IDs, split assignment, normalization, 13-gram construction, SHA-256 membership, tokenizer, EOD behavior, ordering, truncation, or token count. It stages a byte-identical immutable index on native WSL ext4, reuses one read-only/query-only SQLite connection, writes bounded contiguous uint16 chunks, and adds fsynced output-only progress records. Focused byte-equivalence checks passed for clean/contaminated text, normalization, source switching, EODs, dedup skips, mid-document cutoff, and multiple output-buffer boundaries.

A 15M-ID non-scientific proxy completed with exact 2:1 accounting and 107,620 accepted prediction tokens/s end-to-end. It is a systems measurement only, not an EXP-020 data artifact or prefix proof. The controlled real-document screening comparison preserved all 200 decisions and improved from 3.82 documents/s under legacy DrvFS/per-document connections to 15.83 documents/s under native persistent SQLite (4.14x). The final vectorized bounded writer was byte-identical and measured 10.98M IDs/s versus 10.12M IDs/s for legacy scalar memmap assignment (+8.6%). Despite these gains, the end-to-end projection is about 18.6 hours before final hash/prefix verification, above the frozen <=12-hour performance gate. No second full rebuild is authorized until additional systems profiling/optimization passes that gate.

## EXP-020 data-preparation gate amendment and RAM-index proxy (2026-09-04)

The original <=12-hour raw-materialization systems gate failed and was explicitly amended before another full build; it was operational only and did not change any scientific gate or preregistration. The exact RAM-resident index mode loads the immutable 11,664,298 32-byte SQLite values with no approximation, preserved complete screening decisions and the 15M proxy stream SHA, and is recorded as an operational manifest field. The authorized 300M systems proxy completed with exact frozen configuration, tokenizer, ordering, screening, global deduplication, and 2:1 accounting. Its 300,000,000 prediction-token materialization took 976.657 s at 307,170 accepted prediction tokens/s, projecting 6.51 raw hours for 7.2B. It used native ext4 scratch and reached approximately 6.27 GiB observed RSS without a pathological growth trend. It is not a scientific data artifact, does not satisfy either frozen prefix gate, and authorizes no full build by itself; final human authorization remains required.
